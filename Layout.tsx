  ...
  const navItems = isPrivileged
    ? [
        ...
        ...(isAdmin ? [
          ...
          { id: 'salary', label: 'Payroll', icon: '💰' }
        ] : []),
        { id: 'word-king', label: 'Word King', icon: '👑' },
        { id: 'create', label: 'Create Report', icon: '📝' },
        { id: 'interview', label: 'Strategy', icon: '🤝' },
        { id: 'mock', label: 'Mock Exams', icon: '🏆' },
        { id: 'messages', label: 'Requests', icon: '💬', badge: reportsWithAction },
        { id: 'students', label: 'Students', icon: '👥' },
        ...(isAdmin ? [{ id: 'settings', label: 'Settings', icon: '⚙️' }] : []),
      ]
    : [
        { id: 'dashboard', label: 'Study Base', icon: '📈' },
        ...(role !== 'parent' ? [{ id: 'word-king', label: 'Word King', icon: '👑' }] : []),
    ...
    <div className="flex flex-col md:flex-row h-screen bg-[#f8fafc] overflow-hidden">
      ...
      `}>
        ...
        <div className="p-4 mt-auto border-t border-white/5 bg-black/10 shrink-0">
          ...
          <button
            onClick={onLogout}
            className="w-full flex items-center justify-center gap-2 text-xs bg-white/5 hover:bg-rose-500/10 text-slate-400 hover:text-rose-400 px-4 py-3 rounded-xl transition-all duration-200 font-bold border border-white/5 hover:border-rose-500/20 active:scale-95"
          >
            Sign Out
          </button>
        </div>
