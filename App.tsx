...
const App: React.FC = () => {
  ...
  const handleSaveIQ = async (score: number, breakdown: any, analysis: string) => {
    ...
    const newIQ: IQResult = {
      ...
      score,
      estimatedIQ: Math.round(100 + (score - 50) * 0.8), // 簡易換算
      breakdown,
      aiAnalysis: analysis
    };
    ...
    if (student) {
      ...
      const newStudents = students.map(s => s.id === student.id ? { ...s, iqHistory: updatedHistory } : s);
      setStudents(newStudents);
      saveToLocal('students', newStudents);
      if (supabase) await supabase.from('students').update({ iq_history: updatedHistory }).eq('id', student.id);
    }
  };

  // Fix: Create a proper handler for AdminConfig updates that supports Partial<AdminConfig> and handles persistence.
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      case 'dashboard': return <Dashboard reports={reports} students={students} instructors={instructors} role={currentUser.role} mockExams={mockExams} currentUserStudent={activeStudent} currentUserId={currentUser.id} allSessions={allSessions} onLogSession={(s) => setAllSessions([...allSessions, s])} timetable={timetable} onUpdateTimetable={setTimetable} />;
      case 'create': return <ReportForm students={students} currentUser={currentUser} onSave={(r) => setReports([r, ...reports])} />;
      case 'reports': return <ReportList reports={reports} students={students} currentUser={currentUser} onAddMessage={() => {}} onDeleteMessage={() => {}} onMarkResolved={() => {}} />;
      case 'word-king': return <WordKing classroomBest={adminConfig.wordKingClassroomRecord} classroomHolder={adminConfig.wordKingClassroomHolder} userId={currentUser.id} onNewClassroomRecord={(record, holder) => {}} />;
      case 'iq-test': return <IQTest studentName={currentUser.name} grade={activeStudent?.grade || ""} onComplete={handleSaveIQ} />;
      case 'interview': return <InterviewCenter students={students} reports={reports} mockExams={mockExams} adminConfig={adminConfig} />;
      case 'students': return <StudentCenter students={students} reports={reports} allSessions={allSessions} currentUser={currentUser} onAddMessage={() => {}} onDeleteMessage={() => {}} onMarkResolved={() => {}} />;
      case 'timetable': return <TimetableManager timetable={timetable} students={students} instructors={instructors} onUpdate={setTimetable} />;
      ...
      <div className="bg-white p-12 rounded-[3rem] shadow-2xl text-center max-w-sm w-full border border-slate-100">
        ...
        {authStep === 'role-selection' ? (
          <div className="space-y-4">
            <button onClick={() => { setLoginRole('student'); setAuthStep('credentials'); }} className="w-full py-6 bg-indigo-600 text-white rounded-[2rem] font-black shadow-xl">生徒・保護者</button>
            <button onClick={() => { setLoginRole('instructor'); setAuthStep('credentials'); }} className="w-full py-6 bg-slate-800 text-white rounded-[2rem] font-black shadow-xl">講師</button>
            <button onClick={() => { setLoginRole('admin'); setAuthStep('credentials'); }} className="text-xs text-slate-300 font-bold uppercase mt-8 block mx-auto">Admin</button>
          </div>
        ) : (
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="text" value={loginId} onChange={e => setLoginId(e.target.value)} placeholder="ID" className="w-full px-5 py-4 bg-slate-50 border-2 rounded-2xl font-bold outline-none focus:border-indigo-500" />
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Password" className="w-full px-5 py-4 bg-slate-50 border-2 rounded-2xl font-bold outline-none focus:border-indigo-500" />
            <button type="submit" className="w-full py-5 bg-indigo-600 text-white rounded-[1.5rem] font-black shadow-xl">ログイン</button>
            <button onClick={() => setAuthStep('role-selection')} className="text-xs text-slate-400 font-bold">戻る</button>
          </form>
        )}
      </div>
    </div>
